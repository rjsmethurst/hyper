...
def sample(ndim, nwalkers, nsteps, burnin, start, w, ur, sigma_ur, nuvu, sigma_nuvu, age, pd, ps, dr7):
...
#        raise SystemExit('Number of ages does not coincide with number of galaxies...')
    ...
    #N.save('lnprob_burnin_'+str(int(dr7))+'_'+str(time.strftime('%H_%M_%d_%m_%y'))+'.npy', lnp)
    samples = sampler.chain[:,:,:].reshape((-1,ndim))
    samples_save = 'samples_burn_in_'+str(int(dr7))+'_'+str(time.strftime('%H_%M_%d_%m_%y'))+'.npy'
    N.save(samples_save, samples)
    #walker_plot(samples, nwalkers, burnin, dr7)
    sampler.reset()
    print 'RESET'
    # main sampler run
    sampler.run_mcmc(pos, nsteps)
    #lnpr = sampler.flatlnprobability
    #N.save('lnprob_run_'+str(int(dr7))+'_'+str(time.strftime('%H_%M_%d_%m_%y'))+'.npy', lnpr)
    samples = sampler.chain[:,:,:].reshape((-1,ndim))
    samples_save = 'samples_'+str(int(dr7))+'_'+str(time.strftime('%H_%M_%d_%m_%y'))+'.npy'
    N.save(samples_save, samples)
    #fig = triangle.corner(samples, labels=[r'$ t_{q} $', r'$\tau$'])
    #fig.savefig('triangle_t_tau_'+str(int(dr7))+'_'+str(time.strftime('%H_%M_%d_%m_%y'))+'.pdf')
    return samples, samples_save


#Define function to plot the walker positions as a function of the step
def walker_plot(samples, nwalkers, limit, dr7):
    ...
    save_fig = 'walkers_steps_'+str(int(dr7))+'_'+str(time.strftime('%H_%M_%d_%m_%y'))+'.pdf'
